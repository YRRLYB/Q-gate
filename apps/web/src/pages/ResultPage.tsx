...
export function ResultPage() {
  ...
  const wrongItems = useMemo(
    () => result?.graded.filter((item) => !item.correct) ?? [],
    [result],
  );
  ...
  return (
    <Frame
      ...
      title={
        result?.passed ? settings.result.passTitle : settings.result.failTitle
      }
      subtitle={
        result?.passed
      ...
      aside={
        result ? (
          <div
            className={`result-hero-aside ${result.passed ? "is-pass" : "is-fail"}`}
          >
            <span className="result-hero-kicker">
              {result.passed ? "APPLICATION READY" : "REVIEW NEEDED"}
            </span>
            ...
            <p>
              {result.passed
                ? `已达到及格线 ${result.passScore}，可以继续完成进群申请。`
                : `距离及格线 ${result.passScore} 还差一点，先把错题看完再重答。`}
            </p>
          </div>
        ) : null
      }
          ...
          {result ? (
            <>
              <div className="result-meta-strip">
                <span>
                  答对点数 {result.earnedPointsTotal} / {result.totalPoints}
                </span>
                <span>及格线 {result.passScore}</span>
                <span>
                  {wrongItems.length === 0
                    ? "本次无错题"
                    : `错题 ${wrongItems.length} 道`}
                </span>
              </div>
              ...
              {result.passed ? (
                <>
                    ...
                    <div className="verification-panel-grid">
                      <div className="verification-main-block">
                        <div className="verification-meta-row">
                          ...
                          <span className="verification-security-tag">
                            安全时效中
                          </span>
                        </div>
                        ...
                        <p>
                          {result.expiresAt
                            ? `请在有效期内填写到进群申请问题。截止时间：${new Date(result.expiresAt).toLocaleString("zh-CN")}`
                            : "验证码有效期未提供"}
                        </p>
                      </div>
                      <div
                        className={`verification-timer-hero ${remainingMs <= 300000 ? "is-urgent" : ""}`}
                      >
                        <span className="verification-timer-label">
                          安全倒计时
                        </span>
                        ...
                        <p>
                          {remainingMs <= 300000
                            ? "剩余时间不多了，建议现在就去填写申请问题。"
                            : "验证码仍在有效期内，可直接用于进群申请验证。"}
                        </p>
                      </div>
                    </div>
                  </div>
                  <div className="button-row split-actions result-action-row">
                    ...
                    >
                      {copied
                        ? settings.result.copiedButton
                        : settings.result.copyButton}
                    </button>
                  ...
                  {copyError ? (
                    <p className="muted result-copy-note">{copyError}</p>
                  ) : null}
                </>
              ) : (
                <div className="button-row split-actions">
                  <p className="muted">
                    看完下面的错题解析后，再重新开一轮通常会更稳。
                  </p>
                  <Link
        ...
        {result ? (
          <article className="card result-review-card polished-card">
            <div className="section-head">
              ...
              <h2>
                {wrongItems.length === 0
                  ? settings.result.noWrongTitle
                  : settings.result.reviewTitle}
              </h2>
            </div>
            {wrongItems.length === 0 ? (
              ...
              <div className="review-list">
                {wrongItems.map((item, index) => {
                  ...
                  return (
                    <section
                      key={item.id}
                      className={`review-card review-collapsible ${opened ? "is-open" : ""}`}
                    >
                      <div className="review-card-head review-card-head-tight">
                        ...
                        <div>
                          <h3>{item.prompt}</h3>
                          <p>
                            {item.earnedPoints} / {item.points} 分
                          </p>
                        </div>
                        <button
                          className="secondary-button review-toggle"
                          type="button"
                      ...
                      {opened ? (
                        <div className="review-detail-stack">
                          ...
                          <div className="review-answer-block success">
                            ...
                            <p>
                              为保护题库安全，结果页不会公开显示参考答案；如需复核，请联系管理员。
                            </p>
                          </div>
                        </div>
                      ) : null}
